...
public class ImageController {
    ...
    // This method checks if the current logged in user is the 
    //same as the owner of the image and returns True / False
    private Boolean isImageOwner(Integer imageId, HttpSession session) {
        Image curImgData = imageService.getImage(imageId);
        Integer currImgOwId = curImgData.getUser().getId();

        User loggedInUser = (User) session.getAttribute("loggeduser");
        Integer loggedInUserId = loggedInUser.getId();

        return loggedInUserId.equals(currImgOwId);
